    ...
    <h4 style={{padding:'25px'}}>Admin Log In</h4>
      ...
      <form method="post" action="" >
        <div class="form-group">
          <input type="text" id="username" name="username" placeholder="User Name" class="form-control" style={{width:'300px'}} required=""/>
        </div>
        <div class="form-group">
          <input type="password" id="password" name="password" placeholder="Password" style={{width:'300px'}} class="form-control" required=""/>
        </div>
        <div>
          <button type="submit" name="btn-login" onClick={handleclick} class="btn btn-primary w-100">Log In</button>
        </div>
        <div className='text-center'>
